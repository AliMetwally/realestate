<div class="row"> 
    <form target="_blank" action="<?= base_url('reports/rep_separated_property') ?>" method="get" id="deal_property_form">
        ...
        <div class="form-group">
            ...
            <input class="form-control" id="client_phone_empty"  name="client_phone_empty" type="text">                                
        </div>
        <!-- property type -->
        ...
        <div class="col-md-2">
            <a id="non_exist_property" class="btn btn-block btn-warning btn-flat" data-dismiss="modal" style="margin-top: 25px"> حفظ البحث<i class="glyphicon glyphicon-save"></i></a>        
        </div>
...
//         load the towers 
        ...
        if (display_mode == 1) { // in case of manager - display the actions
            property_result_url = '<?= base_url('search_property/get_properties_actions') ?>';
        } else if (display_mode == 2) // in case of sales follow up
        {
            property_result_url = '<?= base_url('search_property/getPropertiesTable') ?>';
        }
        ...
        $('#non_exist_property').click(function () {
            property_non_exist = '<?= base_url('search_property/nonExistProperty') ?>';
            $.ajax({
                url: property_non_exist,
                type: 'get',
                sync: false,
                data: $('#deal_property_form').serialize(),
                success: function (data) {
